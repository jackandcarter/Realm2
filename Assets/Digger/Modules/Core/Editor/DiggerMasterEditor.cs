...
namespace Digger.Modules.Core.Editor
{
    ...
    public class DiggerMasterEditor : UnityEditor.Editor
    {
        ...
        private void ApplyBiomeToTerrain(DiggerSystem digger, BiomePaintOverrides overrides, HashSet<DiggerSystem> targetDiggers, float seamBlendWidth)
        {
            ...
            ApplySeamSmoothToTerrainForDigger(digger, targetDiggers, minChunkX, maxChunkX, minChunkZ, maxChunkZ, seamBlendChunks);
        }
        ...
        private void ApplyBiomeToRegion(TerrainRegion region, HashSet<DiggerSystem> diggers, BiomePaintOverrides overrides, float seamBlendWidth)
        {
            ...
            var regionDiggerSet = new HashSet<DiggerSystem>(regionDiggers);
            foreach (var digger in regionDiggers) {
                ...
                ApplyBiomeToRegionForDigger(region, digger, overrides, minChunkX, maxChunkX, minChunkZ, maxChunkZ, regionDiggerSet);
            }

            var seamBlendChunks = GetSeamBlendChunkCount(region.GetChunkSize(), seamBlendWidth);
            if (seamBlendChunks > 0) {
                foreach (var digger in regionDiggers) {
                    ApplySeamSmoothToRegionForDigger(region, digger, regionDiggerSet, minChunkX, maxChunkX, minChunkZ, maxChunkZ, seamBlendChunks);
                }
            }
        }
        ...
        private void ApplyBiomeToRegionForDigger(TerrainRegion region, DiggerSystem digger, BiomePaintOverrides overrides,
            int minChunkX, int maxChunkX, int minChunkZ, int maxChunkZ, HashSet<DiggerSystem> targetDiggers)
        {
            var terrain = digger.Terrain;
            var terrainSize = terrain.terrainData.size;
            var chunkSize = region.GetChunkSize();
        ...
        private void ApplySeamSmoothToRegionForDigger(TerrainRegion region, DiggerSystem digger, HashSet<DiggerSystem> targetDiggers,
            int minChunkX, int maxChunkX, int minChunkZ, int maxChunkZ, int seamBlendChunks)
        {
            if (seamBlendChunks <= 0) {
                return;
            }
            ...
            var chunkSize = region.GetChunkSize();
            var brushRadius = chunkSize * 0.5f;
            ...
            int minChunkX, int maxChunkX, int minChunkZ, int maxChunkZ, int seamBlendChunks)
        {
            ...
            var terrainSize = terrain.terrainData.size;
            var terrainCenterY = terrain.transform.position.y + terrainSize.y * 0.5f;
            var maxScale = Mathf.Max(digger.HeightmapScale.x, digger.HeightmapScale.z);
            var brushRadius = digger.SizeOfMesh * 0.5f * maxScale;
            var brushSize = new Vector3(brushRadius, brushRadius, brushRadius);
        ...
        public static void RemoveDiggerFromTerrains()
        {
            ...
            var diggerMasters = FindObjectsByType<ADiggerMonoBehaviour>(FindObjectsInactive.Exclude, FindObjectsSortMode.None);
